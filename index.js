...
function getFullNames(runners) {
  const namesList = [];
  runners.forEach(runners => namesList.push(`${runners.last_name}, ${runners.first_name}`));
  return namesList;
}
...
function firstNamesAllCaps(runners) {
  const capNames = [];
  runners.forEach(runners => capNames.push(runners.first_name.toUpperCase()))
  return capNames;
}
...
function tallyUpDonations(runners) {
  const total = runners.reduce((accumulator, currentValue) => {
